...
static inline void bpf_trampoline_inc_progs(struct bpf_prog *prog,
...
int bpf_trampoline_link_prog(struct bpf_prog *prog)
{
	...
	bpf_trampoline_inc_progs(prog, kind);
	err = bpf_trampoline_update(prog);
	if (err) {
		hlist_del(&prog->aux->tramp_hlist);
		bpf_trampoline_dec_progs(prog, kind);
	}

	/* This is the first program to be attached to the LSM hook, the hook
	 * needs to be enabled.
	 */
	if (prog->type == BPF_PROG_TYPE_LSM && tr->nr_lsm_progs == 1)
