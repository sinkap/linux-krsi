...
struct bpf_local_storage {
	...
	union {
		struct sock *sk;	/* The sk that owns the the above "list"
					 * of bpf_sk_storage_elem.
					 */
		struct inode *inode;
	};
	struct rcu_head rcu;
...
static void __unlink_selem(struct bpf_local_storage_elem *selem)
{
	struct bpf_local_storage *local_storage;
	bool free_local_storage = false;

	if (unlikely(!selem_linked_to_node(selem)))
		/* selem has already been unlinked from it's owner */
		return;
...
bpf_local_storage_alloc(struct bpf_local_storage_map *smap)
{
	...
	local_storage = kzalloc(sizeof(*local_storage), GFP_ATOMIC | __GFP_NOWARN);
	if (!local_storage)
		return NULL;

	INIT_HLIST_HEAD(&local_storage->list);
	raw_spin_lock_init(&local_storage->lock);
...
#define MAX_VALUE_SIZE							\
	min_t(u32,							\
	      (KMALLOC_MAX_SIZE - MAX_BPF_STACK - sizeof(struct bpf_local_storage_elem)), \
...
bpf_local_storage_map_alloc(union bpf_attr *attr)
{
	...
	/* Use at least 2 buckets, sk_select_bucket() is undefined behavior with 1 bucket */
